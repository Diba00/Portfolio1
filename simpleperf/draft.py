    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            ...
            with conn:
                ...
                throughput = total_bytes * 8 / duration /1000000 # Deler på 8 antall tall i bytes og deler med 1 mill Mbps #Rate
                #interval_str = f"{duration:.2f} s" #Interval
                interval_str = f"{start_time:.2f}-{current_time:.2f} s" #Interval
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        while True:
            ...
            with conn:
                ...
                # Initialize start time
                start_time = 0 #time.monotonic()
                current_time = start_time # Initialize current time outside the while loop
                ...
                while True:
                    ...
                    # Update current time inside the while loop
                    current_time = time.monotonic()
...
def client(host, port, duration):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        ...
        print(f"{host}:{port:<10}{interval_str:>15}  {total_data_mb:>10.0f} MB   {throughput:>.2f} Mbps")
